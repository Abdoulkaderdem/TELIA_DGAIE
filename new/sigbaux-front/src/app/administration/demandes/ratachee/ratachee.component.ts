...
@Component({
  selector: 'app-ratachee',
  templateUrl: './ratachee.component.html',
  providers: [MessageService, ConfirmationService]
})
export class RatacheeComponent {
  requests: Request[] = [];
  ...
  ngOnInit() {
    this.requestService.getRequestsByStatus("BATIMENT_FOUND").subscribe(data => (this.requests = data));
  ...
  valideRequestConfirmation(request: Request) {
    ...
    this.request = { ...request };
    this.router.navigate([`/demandes/search-building/${this.request.id}`]);
  }
  ...
  validateRequest() {
    this.requestService.changeRequestStatus('held',this.request.id).subscribe(() => {
      this.requestService.getRequestsByStatus("BATIMENT_FOUND").subscribe(data => (this.requests = data));
      this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'demande retenue avec succes !', life: 3000 });
      this.validateRequestDialog = false;
      this.request = {} as Request;
    });
