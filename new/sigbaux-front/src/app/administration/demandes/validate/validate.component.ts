...
export class ValidateComponent {
  ...
  validateRequest() {
    this.requestService.changeRequestStatus('send',this.request.id).subscribe(() => {
      this.requestService.getRequestsByStatus("VALIDATE").subscribe(data => (this.requests = data));
      this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'demande envoyée avec succes !', life: 3000 });
      this.validateRequestDialog = false;
      this.request = {} as Request;
    });
