...
export class SendedComponent {
  ...
  cancelRequestDialog: boolean = false;
  ...
  cancelRequestConfirmation(request: Request) {
  ...
  validateRequest() {
    this.requestService.changeRequestStatus('approval',this.request.id).subscribe(() => {
      this.requestService.getRequestsByStatus("SEND").subscribe(data => (this.requests = data));
      this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'demande approuvée avec succes !', life: 3000 });
      this.validateRequestDialog = false;
      this.request = {} as Request;
    });
  ...
  cancelRequest() {
    this.requestService.changeRequestStatus('complement',this.request.id).subscribe(() => {
      this.requestService.getRequestsByStatus("SEND").subscribe(data => (this.requests = data));
      this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'demande rejetée avec succes !', life: 3000 });
