...
@Component({
  selector: 'app-indemnisation-contract',
  templateUrl: './indemnisation-contract.component.html',
  providers: [MessageService, ConfirmationService]
})
export class IndemnisationContract {
  ...
  searchSuccess(response: any): void {
    ...
    if(this.contract?.status == 'ENABLE'){
      this.messageService.add({
        severity:'warn', 
        summary: 'Echec', 
        detail: 'Ce contrat est toujours actif !', 
        life: 3000
      });
    }
    else {
      this.contractService.getIndemnity(this.contract!.id)
        .subscribe(
          (data) => {
            this.indemnity = data;
            console.log(data);
  ...
  onSubmit(): void {
    ...
    this.contractService.terminateContract(contractId,motif)
    .subscribe(response => {
      console.log('Invoice submitted successfully', response);
      this.messageService.add({severity:'success', summary: 'Success', detail: 'Succès !'});
    }, error => {
      console.log(error);
        this.messageService.add({severity:'error', summary: 'Echec', detail: 'Echec !'});
    });
  }
